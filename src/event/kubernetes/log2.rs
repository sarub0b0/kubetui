...
/// httpエラー時のLogParamsの生成に使用する
#[derive(Copy, Clone)]
enum FetchLogStreamState {
    /// 未取得
    NotFetching,
    /// 途中まで取得している
    Fetching,
...
struct FetchLogStreamError {
    error: kube::Error,
    state: FetchLogStreamState,
}

impl FetchLogStream {
    async fn fetch_log_stream(&self, lp: &LogParams) -> Result<(), FetchLogStreamError> {
        let mut state = FetchLogStreamState::NotFetching;

        let mut logs = self
            .pod_api
            .log_stream(&self.pod_name, lp)
            .await
            .map_err(|e| FetchLogStreamError { error: e, state })?
            .lines();

        let prefix = self.prefix.clone().map(|p| p + " ").unwrap_or_default();

        while let Some(line) = logs.try_next().await.map_err(|e| FetchLogStreamError {
            error: kube::Error::ReadEvents(e),
            state,
        })? {
            let mut buf = self.buf.write().await;
...
impl Worker for FetchLogStream {
    ...
    async fn run(&self) -> Self::Output {
        ...
        while let Err(e) = self.fetch_log_stream(&lp).await {
            match e.error {
                kube::Error::Api(_) => todo!(),
                kube::Error::HyperError(_) => todo!(),
            ...
            match e.state {
                FetchLogStreamState::NotFetching => todo!(),
                FetchLogStreamState::Fetching => todo!(),
            }
        }
