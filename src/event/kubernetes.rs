...
use self::{
    ...
    inner::Inner,
    log::{LogHandlers, LogStreamMessage, LogWorker, LogWorkerBuilder},
...
impl KubeWorker {
    ...
    pub fn run(&self) -> Result<()> {
        ...
        let ret: Result<()> = match Runtime::new() {
            Ok(rt) => {
                match rt.block_on(Self::inner(self.clone())) {
                    Ok(_) => Ok(()),
                    Err(e) => Err(e),
                }
            }
                    ...
                    match ev {
                        ...
                        Kube::LogStream(LogStreamMessage::Request { namespace, name }) => {
                            ...
                            log_stream_handler = Some(
                                LogWorker::new(tx, kube_client.clone(), namespace, name).spawn(),
                            );
