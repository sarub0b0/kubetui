        ...
        loop {
            ...
            if let Some(mut override_namespaces) = override_namespaces.take() {
                ...
                if not_found_namespaces.is_empty() {
                    crate::logger!(error, "Using namespaces: {found_namespaces:?}");
                    stored_target_namespaces = found_namespaces;
                } else {
                    crate::logger!(error, "Not found namespaces: {not_found_namespaces:?}. Falling back to stored namespaces: {stored_target_namespaces:?}");
                }
            }
