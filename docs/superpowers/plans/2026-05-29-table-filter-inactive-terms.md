...
```rust
    #[test]
    fn matches_skips_include_when_column_absent_from_header() {
        // version 列が header に無い → version の include はスキップされ行は残る
    ...
    #[test]
    fn matches_skips_exclude_when_column_absent_from_header() {
        let header = vec!["NAME".to_string(), "STATUS".to_string()];
        let item = make_item(&["gke-a", "Ready"]);
    ...
    fn matches_still_applies_present_columns() {
        ...
        assert!(pred.matches(&ready, &header));
        assert!(!pred.matches(&not_ready, &header));
    }
```
...
Expected: `matches_skips_include_when_column_absent_from_header` が FAIL（現状は absent 列を空文字扱いし include が全行を弾くため）。
...
```rust
    ...
    ///
    /// A constraint whose column is not present in `header` (e.g. the column
    /// was hidden via the column dialog) is **inactive**: it is skipped rather
    /// than failing the row, so the remaining visible-column constraints still
    /// apply and rows stay visible.
    pub fn matches(&self, item: &TableItem, header: &[String]) -> bool {
        ...
        for (col, patterns) in &self.column_includes {
            let Some(idx) = column_index(header, col) else {
                continue; // inactive: column not in current header
            };
...
```rust
/// Index of the header column whose normalized name equals `col_name`'s, or
/// `None` if no such column is currently displayed (the constraint is inactive).
// TODO(perf): column_index() is called per column × per row × per render. Each
// invocation re-normalizes the entire header. If profiling shows this in the
// hot path, pre-compute a column-name → index map at filter_state set time.
fn column_index(header: &[String], col_name: &str) -> Option<usize> {
    let key = normalize_column_name(col_name);
    header.iter().position(|h| normalize_column_name(h) == key)
}
...
```rust
    ...
    fn inactive_columns(&self) -> Vec<String> {
        ...
        let header_norms: Vec<String> = self
            .items
            .header()
            .original()
            .iter()
        ...
        let mut cols: Vec<String> = state
            ...
            .filter(|c| !header_norms.contains(c))
            .cloned()
            .collect();
        cols.sort();
        cols.dedup();
...
注: `filter_state` のキーは parser が `normalize_column_name` で正規化済みなので `header_norms.contains(c)` でそのまま比較できる。`normalize_column_name` は `table.rs` の `pub use filter_applicator::{…}` 再エクスポートでスコープ内（Phase A で追加済み）。
